...
export interface CoverInput {
  ...
  extra?: string;
  // Honest, posting-matched competencies to mirror in paragraph 1 (from tailor.ts).
  highlights?: string[];
}
...
export function generateCoverLetter(input: CoverInput): string {
  ...
  // One honest, posting-specific sentence mirroring the role's stated needs.
  const matchSentence = highlights.length
    ? ` Your posting's focus on ${joinList(highlights)} lines up directly with my hands-on experience.`
    : '';

  if (input.track === 'A') {
    return [
      'Dear Hiring Manager,',
      `I am applying for ${rolePhrase} at ${employer}. My Architecture: Construction Engineering Technology diploma gave me hands-on AutoCAD and Revit experience across three semesters of design projects, and I pair that drafting foundation with a careful, detail-focused approach that fits this role.${matchSentence}${extraSentence}`,
  ...
  return [
    'Dear Hiring Manager,',
    `I am applying for ${rolePhrase} at ${employer}. I bring over four years of hands-on manufacturing and production experience, and I am ready to contribute safely and reliably from my first shift.${matchSentence}${extraSentence}`,
